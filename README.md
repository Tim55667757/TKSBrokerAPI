...
- Закрывать ранее открытые позиции полностью (кроме заблокированных объёмов), указав конкретный инструмент или список инструментов через их тикеры или FIGI;
  - ключи `--close-trade` или `--cancel-trade`;
...
В первом случае модуль будет доступен в консоли через команду `tksbrokerapi`, а во втором случае вам придётся запускать его как обычный python-скрипт, через `python TKSBrokerAPI.py` из каталога с исходным кодом.
...
Сервис TINKOFF INVEST API использует для аутентификации токен. Токен — это набор символов, в котором зашифрованы данные о владельце, правах доступ и прочая информация, необходимая для авторизации в TINKOFF INVEST API. Токен необходимо передавать на сервер с каждым сетевым запросом.
...
```commandline
$ tksbrokerapi --deals 2022-07-01 2022-07-27 --output deals.md

TKSBrokerAPI.py     L:1972 INFO    [2022-07-27 18:13:18,960] # Client's operations

* **Period:** from [2022-07-01] to [2022-07-27]

## Summary (operations executed only)

| 1                          | 2                             | 3                            | 4                    | 5
|----------------------------|-------------------------------|------------------------------|----------------------|------------------------
| **Actions:**               | Operations executed: 30       | Trading volumes:             |                      |
|                            |   Buy: 16 (53.3%)             |   rub, buy: -25907.12        |                      |
|                            |   Sell: 14 (46.7%)            |   rub, sell: +11873.86       |                      |
|                            |                               |   usd, buy: -278.27          |                      |
|                            |                               |   usd, sell: +24.13          |                      |
|                            |                               |                              |                      |
...
TKSBrokerAPI.py     L:1978 INFO    [2022-07-27 18:13:18,975] History of a client's operations are saved to file: [deals.md]
```
...
TKSBrokerAPI.py     L:1972 INFO    [2022-07-27 18:29:15,026] # Client's operations

* **Period:** from [2022-07-24] to [2022-07-27]
...
|                            |   Buy: 6 (75.0%)              |   rub, buy: -23138.58        |                      |
|                            |   Sell: 2 (25.0%)             |   rub, sell: +9206.00        |                      |
|                            |                               |   usd, buy: -207.32          |                      |
|                            |                               |   usd, sell: 0               |                      |
|                            |                               |                              |                      |
| **Payments:**              | Deposit on broker account:    | Withdrawals:                 | Dividends income:    | Coupons income:
...
TKSBrokerAPI.py     L:1978 INFO    [2022-07-27 18:29:15,032] History of a client's operations are saved to file: [report.md]
```
...
TKSBrokerAPI.py     L:1972 INFO    [2022-07-27 18:29:59,035] # Client's operations

* **Period:** from [2022-07-20] to [2022-07-27]

## Summary (operations executed only)

| 1                          | 2                             | 3                            | 4                    | 5
|----------------------------|-------------------------------|------------------------------|----------------------|------------------------
| **Actions:**               | Operations executed: 10       | Trading volumes:             |                      |
|                            |   Buy: 7 (70.0%)              |   rub, buy: -23138.58        |                      |
|                            |   Sell: 3 (30.0%)             |   rub, sell: +9206.00        |                      |
|                            |                               |   usd, buy: -208.06          |                      |
|                            |                               |   usd, sell: +2.14           |                      |
|                            |                               |                              |                      |
| **Payments:**              | Deposit on broker account:    | Withdrawals:                 | Dividends income:    | Coupons income:
...
TKSBrokerAPI.py     L:1978 INFO    [2022-07-27 18:29:59,045] History of a client's operations are saved to file: [report.md]
```
...
<details>
  <summary>Команда для совершения сделки покупки и выставления ордеров тейк-профит и стоп-лосс</summary>
...
<details>
  <summary>Команда для продажи ранее купленного инструмента (без указания SL/TP ордеров, но с подробными логами</summary>
...
❗ Важно отметить, что в текущей версии TKSBrokerAPI открытые позиции по валютам не будут закрыты с ключом `--close-all` (`--cancel-all`). Это связано с тем, что остальные инструменты могут использовать различные базовые валюты и сначала нужно закрыть их. Кроме того, пользователь может не хотеть сокращения своих валютных позиций, чтобы покупать на эти средства другие инструменты в будущем. При необходимости позиции по валютам можно закрыть вручную, используя ключи `--buy`, `--sell`, `--close-trade` или `--close-trades`.
...
- если инструмент имеется в списке открытых позиций, то проверить:
   - если текущая цена выше средней цены позиции на 3%, то выставить тейк-профит как отложенный рыночный ордер ещё на 0.05% выше текущей цены, чтобы позиция закрылась с профитом с большой вероятностью в течении текущей сессии.
