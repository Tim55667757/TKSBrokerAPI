...
* [В работе] [#13](https://github.com/Tim55667757/TKSBrokerAPI/issues/13) Для уменьшения количества запросов к серверу, добавлена возможность создания локального дампа с информацией по биржевым инструментам. Дамп можно использовать, чтобы не загружать данные по инструментам при первичной инициализации класса `TinkoffBrokerServer`. Добавлен метод `DumpInstruments()` и ключ `--dump` с помощью которых можно создать первичный дамп-файл.
